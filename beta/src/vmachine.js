...
/*Original function notesFormat with inline conditional */
/* function notesFormat(format) {
   var mssArea = document.getElementById("mssArea");
   var notesPanel = document.getElementById("notesPanel");
   var icons = getElementsByClass("noteicon",mssArea,"span");
   var icons2 = getElementsByClass("noteicon_header",mssArea,"span");
   if (format == "popup") {
      notesPanel.style.display = "none";
      for (v = 0; v < icons.length; v++) {
         icons[v].style.display = "inline";
		 icons2[v].style.display = "inline";
      }
   } else if (format == "inline") {
      for (v = 0; v < icons.length; v++) {
         icons[v].style.display = "none";
		 icons2[v].style.display = "none";
      }
...
/*Modified notesFormat to include noteicon and noteicon_header classes; popup notes is default; Leah M Root SUNY Geneseo  */
 function notesFormat(format) {
   var mssArea = document.getElementById("mssArea");
   var notesPanel = document.getElementById("notesPanel");
   var icons = getElementsByClass("noteicon",mssArea,"span");
...
/*New notesMenu toggle; conditional based on current state of notes visibility, with popup being the visible state. 
if notes are not displayed, onclick will show them. 
If notes are displayed, onclick will hide them. Leah M Root SUNY Geneseo  */
function notesFormatToggle(format){
	var notesMenu_none = document.getElementById("notesMenu_none");
	var notesMenu_popup = document.getElementById("notesMenu_popup");
	if(format=='none'){
		notesMenu_none.style.display = "none";
		notesMenu_popup.style.display = "inline";
	}
	if(format=='popup'){
		notesMenu_none.style.display = "inline";
		notesMenu_popup.style.display = "none";
	}
	notesFormat(format);
...
/*No longer needed, but can be used if it is decided to show biblio panel again */
...
/*actually simple toggle visibility function; Leah M Root based on snippet on css-tricks.com;
this function is used by toggleTOC and toggleHelp and can be called by any future button that needs to toggle a div */ 
